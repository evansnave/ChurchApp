...
$query = "SELECT * FROM fundraising WHERE fundraising_status = 'active' ";
...
if ($count == 0 && empty($rows)) {
    echo "<h3 class=\"text-center text-muted\">Please <strong>Add </strong>Fundraising</h3>";
} else {
    ?>
        ...
        <tbody>
            <?php foreach ($rows as $results) { ?>
                <tr>
                    ...
                    <td><?= strtoupper($results['fundraising_type']) ?> </td>
