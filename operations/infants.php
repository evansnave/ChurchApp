...
if (isset($_POST['btn_action'])) {
    if ($_POST['btn_action'] == 'Add') {
        ...
        $query = "SELECT * FROM `infants` WHERE member_status ='active' AND phone_number = $phone_number";
        ...
        if ($count >= 1) {
            ...
            $query = "INSERT INTO infants (title, fullname, guardian_name, phone_number, residence, gender, age, baptism, dob,year_joined) 
    ...
    if ($_POST['btn_action'] == 'fetch_single') {
        $query = "SELECT * FROM infants WHERE id = :id";
        $statement = $db->prepare($query);
        $statement->execute(
            array(
                ':id'    =>    $_POST["id"]
